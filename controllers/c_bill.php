...
class c_bill extends controller{
    ...
    public function serviceindex() {
        ...
        if($selectedPaymentMethod == 'momo'){
            $billId = $_POST["bill_id"];
            $update = $result->updateStatusOrderService($billId);
            include "api/momo.php";
        ...
        if($selectedPaymentMethod == 'zalopay'){
            ...
            $key2 = 'uUfsWgfLkRLzq6W2uNXTCxrfxs51auny';
            $endpoint ='https://sandbox.zalopay.com.vn/v001/tpe/createorder';
            $redirecturl = 'http://localhost/dormitory_manager/bill/serviceindex';
            $embeddata = [
                "merchantinfo" => "embeddata123",
                "redirecturl" => "$redirecturl?code=" . rand(10,100) . "&order_ticket=" . rand(10,100),
            ];
        ...
        if($selectedPaymentMethod == 'cash'){
            $billId = $_POST["bill_id"];
            $update = $result->updateStatusOrderService($billId);
            header("location: http://localhost/dormitory_manager/bill/serviceindex");
        }
    ...
    public function createservice() {
        $result = new m_bill();
        $services = $result->getAllService();
        $room = $result->getRoomId($_SESSION["login"]["username"]);
        ...
        $this->view("bill/createService", compact("services", "room"));
    }
}